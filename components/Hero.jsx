...
export default function HeroSlider() {
  ...
  const slides = [
    ...
    {
      image: "/hero-background.png", // this image may be too dark — consider adjusting it in a photo editor or replacing with another one.
      title: "SPORTSWEAR",
      subtitle:
        "Dynamic sportswear that empowers your team spirit and comfort.",
    },
  ...
  return (
    <div className="relative w-full h-[50vh] sm:h-[100dvh] bg-[#E6D8C8] overflow-hidden">
      <Swiper
        modules={[Pagination, Autoplay]}
        onSwiper={(swiper) => {
          swiperRef.current = swiper;
        ...
        pagination={{ clickable: true }}
        loop={true}
        autoplay={{ delay: 7000 }} // Increased delay to 7 seconds
        className="w-full h-full"
      >
        {slides.map((slide, index) => (
          <SwiperSlide key={index}>
            <div className="relative w-full h-full bg-[#E6D8C8]">
              <Image
                src={slide.image}
                alt={
                  typeof slide.title === "string" ? slide.title : "Hero Slide"
